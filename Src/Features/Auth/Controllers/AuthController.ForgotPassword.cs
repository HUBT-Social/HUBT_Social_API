...
public partial class AuthController
{
    ...
    {
        var isEmail = request.UserNameOrEmail.Contains("@gmail.com");
        ...
        try
        {
            ...
            {
                var code = await _emailService.CreatePostcodeForgetPasswordAsync(userAgent, user.Email, ipAddress);
                ...
                if (_emailService.MaskEmail(user.Email, out var maskEmail))
                    return Ok(
                        new
                        {
                            Email = maskEmail,
    ...
    public async Task<IActionResult> VerifyOtp([FromBody] OTPRequest request)
    {
        ...
        var currentPostcode = await _emailService.GetCurrentPostCode(userAgent, ipAddress);
    ...
    {
        return await PostcodeHelper.ResendPostcode(HttpContext, Request,
            _emailService.CreatePostcodeForgetPasswordAsync, _emailService, PostcodeType.ForgetPassword);
    }
    ...
    public async Task<IActionResult> UpdatePassword([FromBody] UpdatePasswordRequest request)
    {
        ...
        {
            var User = await _userService.FindUserByEmailAsync(currentEmail.Email);
            if (User != null)
                return await UpdateHelper.HandleUserUpdateForgotPassword(KeyStore.PasswordUpdated,
                    KeyStore.PasswordUpdateError, _userService.UpdatePasswordAsync, request, User);
            return BadRequest(LocalValue.Get(KeyStore.UserNotFound));
        }
