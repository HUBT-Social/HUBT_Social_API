...
namespace HUBTSOCIAL.Src.Features.Chat.Services.ChildChatServices
{
    public class MessageService : IMessageService
    {
        ...
        public async Task<bool> DeleteMessageAsync(string chatRoomId, string messageId)
        {
            ...
            var update = Builders<ChatRoomModel>.Update.PullFilter("Messages", Builders<MessageModel>.Filter.Eq(m => m.Id, messageId));
            await _chatRooms.UpdateOneAsync(cr => cr.Id == chatRoomId, update);
            return true;
        }
        ...
        {
            var chatRoom = await _chatRooms.Find(cr => cr.Id == chatRoomId).FirstOrDefaultAsync();
            return chatRoom?.Messages?.Where(m => m.Content.Contains(keyword, StringComparison.OrdinalIgnoreCase)).ToList() ?? new List<MessageModel>();
        }
    }
}
