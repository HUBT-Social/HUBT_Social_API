    ...
    public class TokenService : ITokenService
    {
        ...
        public async Task<TokenResponse> RefreshToken(RefreshTokenRequest request)
        {
            ...
            if (refreshToken != null)
            {
                ...
                if (!principal.Success) return new TokenResponse
                {
                    Success = false,
                ...
                if (userId == null) return new TokenResponse
                {
                    Success = false,
        ...
        private async Task<string> ReGenerateToken(AUser user)
        {
            ...
            string refreshtoken = GenerateRefreshToken(await _userManager.GetClaimsAsync(user));
        ...
        {
            RefreshToken? existingRefreshToken = await _refreshToken.Find(t => t.UserId == user.Id.ToString()).FirstOrDefaultAsync();

            if (existingRefreshToken == null)
            {
                await _refreshToken.InsertOneAsync(new RefreshToken { AccessToken = accessToken, UserId = user.Id.ToString() });
            }
            else
            {
                UpdateDefinition<RefreshToken>? update = Builders<RefreshToken>.Update.Set(t => t.AccessToken, accessToken);
